...
func Test_teacherDao_GetByID(t *testing.T) {
	...
	_, err := d.IDao.(TeacherDao).GetByID(d.Ctx, testData.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func Test_teacherDao_GetByCondition(t *testing.T) {
	d := newTeacherDao()
	defer d.Close()
	testData := d.TestData.(*model.Teacher)
	...
	_, err := d.IDao.(TeacherDao).GetByCondition(d.Ctx, &query.Conditions{
